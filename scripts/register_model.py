...
def register_model(source_dir: str, run_id: str = None):
    ...
    source_path = Path(source_dir) if Path(source_dir).is_absolute() else base_dir / source_dir
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Register a trained model in the model registry.")
    parser.add_argument("--source-dir", 
                        default="models/phishing",
                        help="Directory containing the trained model and metadata")
    parser.add_argument("--run-id", 
                        help="Optional run ID. If not provided, uses current timestamp")
