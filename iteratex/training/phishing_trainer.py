...
class PhishingTrainer:
    ...
    def load_data(self) -> pd.DataFrame:
        ...
        if target_col is None:
            raise ValueError("Could not find target column in the dataset. Expected one of: 'status', 'phishing', 'is_phishing', 'label', 'class'")
        
        # Convert target to binary (1 for phishing, 0 for legitimate)
        if df[target_col].dtype == 'object':
            # If target is string, convert to binary
            df['label'] = (df[target_col].str.lower().str.contains('phish|malicious|1')).astype(int)
        else:
    ...
    def train(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Log class distribution
            logger.info(f"Class distribution in full dataset: {y.value_counts().to_dict()}")
    ...
    def save_model(self, metrics: Dict[str, Any]) -> None:
        ...
        try:
            ...
            feature_names = list(self.model.feature_names_in_) \
                if hasattr(self.model, 'feature_names_in_') else []
            ...
            metadata = reg_meta.ModelMetadata(
                ...
                hyperparameters={
                    ...
                    'max_depth': self.model.max_depth if hasattr(self.model, 'max_depth') else 10,
                },
...
def print_metrics(metrics: Dict[str, Any]) -> None:
    ...
    for class_name, metrics in class_report.items():
        if isinstance(metrics, dict):  # Skip non-class entries like 'accuracy', 'macro avg', etc.
            rows.append([
                class_name,
